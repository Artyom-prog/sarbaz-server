...
def app_version():
    ...
    try:
        ...
        if not latest or latest_build <= 0:
            return jsonify({"error": "config.latestVersion/latestBuild required"}), 500

        resp = {
            "latestVersion": latest,
            "latestBuild": latest_build,
            "minSupportedVersion": (str(data.get("minSupportedVersion") or "").strip() or None),
            "minSupportedBuild": (int(data.get("minSupportedBuild", 0)) or None)
        }

        # Логирование запроса (необязательно)
